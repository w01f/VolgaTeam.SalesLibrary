namespace SalesLibraries.CloudAdmin.Configuration
{
	class MetaDataConst
	{
		public const string SecurityGroupsDataTag = "library-security";
		public const string CategoriesDataTag = "link-categories";
		public const string SuperFiltersDataTag = "link-super-filters";
		public const string ExternalLibraryLinksDataTag = "external-library-links";
		public const string ExternalShortcutsDataTag = "shortcuts-links";

		public const string LastUpdatePropertyName = "last-update";
		public const string MaxTagsPropertyName = "max-tags";
		public const string CountTagsPropertyName = "tag-count";
	}
}
