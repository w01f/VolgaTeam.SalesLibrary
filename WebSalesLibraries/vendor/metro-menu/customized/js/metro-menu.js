$(document).ready(function ()
{
	...
	$('.menu').not('.level .menu').on('click', function ()
	{
		var el = $(this);
		if ($('.level.open').length == 0)
		{
			openMenu(el);
			$('#content-overlay').removeClass('action-menu-opened').addClass('action-menu-opened');
		}
	});
	$(document).on('click', function (e)
	{
		...
		if ($('.level.open').length > 0)
		{
			...
			$('#content-overlay').removeClass('action-menu-opened');
			setTimeout(function ()
			{
