        ...
        return {
			...
			loadwriteboard: function(params){
				...
				if(typeof(params.chatroommode) != "undefined" && params.chatroommode==1){
					extraQueryString="&chatroommode=1&to="+params.to;
				}
				baseUrl = $.cometchat.getBaseUrl();
				baseData = $.cometchat.getBaseData();
				if(mobileDevice){
				...
				loadCCPopup(baseUrl+'plugins/writeboard/index.php?action=writeboard&'+extraQueryString+'&basedata='+baseData, 'writeboard',"status=0,toolbar=0,menubar=0,directories=0,resizable=1,location=0,status=0,scrollbars=0, width=<?php echo $writebWidth;?>,height=<?php echo $writebHeight;?>",width,height-50,$.ccwriteboard.getLanguage('collaborative_document'),params.force,0,1,1,params.windowMode);
			}
        };
    })();
})(jqcc);
