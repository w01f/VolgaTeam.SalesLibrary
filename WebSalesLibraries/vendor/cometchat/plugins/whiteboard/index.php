...
if(empty($room)){
	if(!empty($chatroommode)){
		...
		$room = md5($channelprefix.'whiteboard_users'.$grp);
	}
}
...
if ($action == 'request') {
	...
	if($optionalmessage == 0){
    	$response = sendMessage($to,$whiteboard_language[2]." <a href='javascript:void(0);' class='accept_White' to='".$userid."' random='".$to."' rooom = '".$room."'chatroommode='0' mobileAction=\"javascript:jqcc.ccwhiteboard.accept('".$userid."','".$to."');\">".$whiteboard_language[3]."</a> ".$whiteboard_language[4],1);
