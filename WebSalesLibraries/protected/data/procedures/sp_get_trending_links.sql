...
in thumbnail_mode varchar(32))
    select
      ...
      case when thumbnail_mode = 'top'
        then (select pv.relative_path from tbl_preview pv where pv.id_container=l.id_preview and ((l.search_format<>'video' and pv.type='png_phone') or (l.original_format='video' and pv.type='mp4 thumb')) order by pv.relative_path limit 1)
      else (select pv.relative_path from tbl_preview pv where pv.id_container=l.id_preview and ((l.search_format<>'video' and pv.type='png_phone') or (l.original_format='video' and pv.type='mp4 thumb')) order by rand() limit 1)
      end as thumbnail
    from tbl_link l
