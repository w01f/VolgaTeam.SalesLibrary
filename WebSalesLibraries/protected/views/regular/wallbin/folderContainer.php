	...
	<? elseif ($style->showCustomTitle && (!$style->hideTopFoldersCustomTitle || ($style->hideTopFoldersCustomTitle && $folder->rowOrder > 0))): ?>
        <style>
            #folder<? echo $folder->id; ?> {
                ...
                color: <? echo (isset($style->textColor)?(Utils::formatColorToHex($style->textColor)):(isset($folder->banner) && $folder->banner->isEnabled ? $folder->banner->foreColor : $folder->headerForeColor)); ?>;
                background-color: <? echo (isset($style->backColor)?(Utils::formatColorToHex($style->backColor)):$folder->headerBackColor); ?>;
                ...
                padding-right: <? echo $style->customTitlePadding->right; ?>px;
            }
