...
<style>
    ...
    #vertical-feed-<? echo $contentBlock->id; ?> .panel-footer .pagination a
    {
        <?if(!empty($contentBlock->viewSettings->style->buttonBackColor)):?>
            background-color: <? echo Utils::formatColorToHex($contentBlock->viewSettings->style->buttonBackColor);?> !important;
        <?endif;?>
        <?if(!empty($contentBlock->viewSettings->style->buttonBorderColor)):?>
            border-color: <? echo Utils::formatColorToHex($contentBlock->viewSettings->style->buttonBorderColor);?> !important;
        <?endif;?>
    }
    ...
    #vertical-feed-<? echo $contentBlock->id; ?> .panel-footer .pagination a .glyphicon
    {
        <?if(!empty($contentBlock->viewSettings->style->buttonIconColor)):?>
            color: <? echo Utils::formatColorToHex($contentBlock->viewSettings->style->buttonIconColor);?> !important;
        <?endif;?>
    }
    ...
    #vertical-feed-<? echo $contentBlock->id; ?> .news-item
    {
        <?if(!empty($contentBlock->viewSettings->style->dividerColor)):?>
            border-bottom-color: <? echo Utils::formatColorToHex($contentBlock->viewSettings->style->dividerColor);?> !important;
        <?endif;?>
    }
</style>
<div id="vertical-feed-<? echo $contentBlock->id; ?>" class="vertical-feed news-block">
    ...
    <div class="panel panel-default"
         style="<? if (!empty($contentBlock->viewSettings->style->outsideBorderColor)): ?>border-color: <? echo Utils::formatColorToHex($contentBlock->viewSettings->style->outsideBorderColor); ?>; -webkit-box-shadow: 0 0 0 <? echo Utils::formatColorToHex($contentBlock->viewSettings->style->outsideBorderColor); ?>; box-shadow: 0 0 0 <? echo Utils::formatColorToHex($contentBlock->viewSettings->style->outsideBorderColor); ?>;<? endif; ?>">
        <div class="panel-heading" style="<? echo $this->renderPartial('landingPageMarkup/style/stylePadding', array('padding' => $contentBlock->viewSettings->style->headerPadding), true); ?><? if ($contentBlock->viewSettings->hideHeader): ?>display: none;<? endif; ?><? if (!empty($contentBlock->viewSettings->style->headerColor)): ?>background-color: <? echo Utils::formatColorToHex($contentBlock->viewSettings->style->headerColor); ?>;border-color: <? echo Utils::formatColorToHex($contentBlock->viewSettings->style->headerColor); ?>;<? endif; ?>">
	        <? if (!empty($contentBlock->viewSettings->icon)): ?>
                <i class="icomoon icomoon-lg <? echo $contentBlock->viewSettings->icon; ?>" style="<? if (!empty($contentBlock->viewSettings->style->headerIconColor)): ?>color: <? echo Utils::formatColorToHex($contentBlock->viewSettings->style->headerIconColor); ?>;border-color: <? echo Utils::formatColorToHex($contentBlock->viewSettings->style->headerColor); ?>;<? endif; ?>"></i>
	        <? else: ?>
                <span class="glyphicon glyphicon-list-alt"></span>
	        <? endif; ?>
            <strong style="<? if (!empty($contentBlock->viewSettings->style->headerTextColor)): ?>color: <? echo Utils::formatColorToHex($contentBlock->viewSettings->style->headerTextColor); ?>;<? endif; ?>"><? echo $contentBlock->viewSettings->title; ?></strong>
        </div>
