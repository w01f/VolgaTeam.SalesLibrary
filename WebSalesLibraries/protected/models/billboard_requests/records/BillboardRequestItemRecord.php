<?
	...
	/**
	 * Class BillboardRequestItemRecord
	 * @property mixed id
	 ...
	 */
	class BillboardRequestItemRecord extends CActiveRecord
	{
		 ...
		 * @param $shortcutId
		 * @param $statusFilter
		 * @return \application\models\billboard_requests\models\ItemListModel[]
		 */
		public function getListItems($listType, $shortcutId, $statusFilter = null)
		{
			...
			if (!empty($statusFilter) && $statusFilter !== \application\models\billboard_requests\models\Dictionaries::StatusItemAll)
				$criteria->addCondition(self::FiledNameStatus . "='" . $statusFilter . "'");

			$itemRecords = $this->findAll($criteria);
			foreach ($itemRecords as $itemRecord)
				$models[] = \application\models\billboard_requests\models\ItemListModel::fromRecord($itemRecord);
		 ...
		 * @param $shortcutId
		 * @param $statusFilter
		 * @return \application\models\billboard_requests\models\ItemListInfo
		 */
		public function getItemListInfo($shortcutId, $statusFilter = null)
		{
			$itemListInfo = new \application\models\billboard_requests\models\ItemListInfo();
		...
		/**
		 * @param $itemId
		 * @return \application\models\billboard_requests\models\ItemEditModel
		 */
		public function getEditModel($itemId)
		{
			$itemRecord = $this->findByPk($itemId);
			if (isset($itemRecord))
				return \application\models\billboard_requests\models\ItemEditModel::fromRecord($itemRecord);
			return null;
		}
		 ...
		 * @param $assignedTo
		 * @param $dateNeeded
		 * @param $content \application\models\billboard_requests\models\Content
		 */
		...
		public static function addItem($ownerId, $title)
		{
			...
			$itemRecord->status = \application\models\billboard_requests\models\Dictionaries::StatusItemIncompleted;
		...
		{
			/** @var $sourceItemRecord BillboardRequestItemRecord */
			...
			if (isset($sourceItemRecord))
			{
				...
				BillboardRequestFileRecord::cloneFiles($itemRecord->id, $sourceItemId);
		...
		public static function deleteItem($itemId)
		{
			...
			BillboardRequestFileRecord::deleteFilesByItem($itemId);
		}
		...
		{
			/** @var BillboardRequestItemRecord $itemRecords */
