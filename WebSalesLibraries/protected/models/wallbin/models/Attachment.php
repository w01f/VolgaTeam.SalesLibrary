	...
	class Attachment
	{
		...
		public function load($attachmentRecord)
		{
			...
			if (isset($attachmentRecord->id_preview))
			{
				...
				$previewRecords = PreviewRecord::model()->findAll('id_container=?', array($attachmentRecord->id_preview));
						...
						case 'png':
							if (isset($this->universalPreview))
								if (isset($this->universalPreview->pngLinks))
								{
									...
									foreach ($this->universalPreview->pngLinks as $link)
									{
										$viewSources[] = array('title' => 'PNG Viewer - ' . ($this->name . ' - Page ' . ($i + 1) . ' of ' . $count), 'href' => $link, 'href_mobile' => $link);
										$i++;
									}
								}
							break;
						case 'png_phone':
							if (isset($this->universalPreview))
								if (isset($this->universalPreview->pngPhoneLinks))
								{
									...
									foreach ($this->universalPreview->pngPhoneLinks as $link)
									{
										$viewSources[] = array('title' => ($this->name . ' - Page ' . ($i + 1) . ' of ' . $count), 'href' => $link, 'href_mobile' => $link);
										$i++;
									}
								}
							break;
						case 'jpeg':
							if (isset($this->universalPreview))
								if (isset($this->universalPreview->jpegLinks))
								{
									...
									foreach ($this->universalPreview->jpegLinks as $link)
									{
										$viewSources[] = array('title' => 'JPEG Viewer - ' . ($this->name . ' - Page ' . ($i + 1) . ' of ' . $count), 'href' => $link);
										$i++;
									}
								}
							break;
						case 'jpeg_phone':
							if (isset($this->universalPreview))
								if (isset($this->universalPreview->jpegPhoneLinks))
								{
									...
									foreach ($this->universalPreview->jpegPhoneLinks as $link)
									{
										$viewSources[] = array('title' => ($this->name . ' - Page ' . ($i + 1) . ' of ' . $count), 'href' => $link);
										$i++;
									}
								}
							break;
