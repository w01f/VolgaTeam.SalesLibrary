<?
	...
	/**
	 * Class MarketingContestFileRecord
	 * @property mixed id
	 ...
	 */
	class MarketingContestFileRecord extends CActiveRecord
	{
		...
		{
			return '{{marketing_contest_file}}';
		}

		/**
		 * @param $itemId
		 * @param $fileType
		 * @return \application\models\marketing_contest\models\FileModel[]
		 */
		public function getModels($itemId, $fileType)
		{
			...
			foreach ($fileRecords as $fileRecord)
				$models[] = \application\models\marketing_contest\models\FileModel::fromRecord($fileRecord);
			return $models;
		}
		...
		public function getModel($fileId)
		{
			...
			if (isset($itemRecord))
				return \application\models\marketing_contest\models\FileModel::fromRecord($itemRecord);
			return null;
		}
		...
		public function getFileContent($fileId)
		{
			...
			if (isset($fileRecord))
			{
				$itemRecord = MarketingContestItemRecord::model()->findByPk($fileRecord->id_item);
		...
		public static function addItem($itemId, $fileName, $fileType, $tempPath)
		{
			...
			$itemRecord = MarketingContestItemRecord::model()->findByPk($itemId);
			if (isset($itemRecord) && !empty($itemRecord->storage_path))
			{
				$folderPath = self::getStoragePath($itemRecord->storage_path);
		...
		public static function cloneFiles($itemId, $sourceItemId)
		{
			...
			/** @var $sourceFileRecords MarketingContestFileRecord[] */
		...
		{
			/** @var $fileRecord MarketingContestFileRecord */
		...
		{
			/** @var MarketingContestFileRecord $fileRecords */
		...
		private static function getMaxItemIndex($itemId, $fileType)
		{
			return Yii::app()->db->createCommand()
				...
				->from('tbl_marketing_contest_file')
