(function ($)
{
	...
	{
		this.viewSelectedFormat = function (itemContent, resolution)
		{
		...
		var runAddPage = function (linkId, title, fileName, fileType)
		{
			...
			{
				$.SalesPortal.EmailManager.addLitePage(linkId);
			});
			...
			var addPageLogo = $('#add-page-logo');
			addPageLogo.find('.page-content').find('li').attr("data-theme", "c").removeClass("ui-btn-up-e").removeClass('ui-btn-hover-e').removeClass('qpage-logo-selected').addClass("ui-btn-up-c").addClass('ui-btn-hover-c');
			addPageLogo.find('.page-content').find('li').first().attr("data-theme", "e").removeClass("ui-btn-up-c").removeClass('ui-btn-hover-c').addClass("ui-btn-up-e").addClass('ui-btn-hover-e').addClass('qpage-logo-selected');

			//set activity tracking
			$('#add-page-info, #add-page-logo, #add-page-tracking, #add-page-pin, #add-page-options').on('pageshow.activity', function ()
			{
				$.ajax({
					type: "POST",
					url: window.BaseUrl + "statistic/writeActivity",
			...
			$("#add-page-logo").find('.page-content').find('li').off('click.activity').on('click.activity', function ()
			{
				$.ajax({
					type: "POST",
					url: window.BaseUrl + "statistic/writeActivity",
			...
			$('#add-page-info-disclaimer,#add-page-tracking-disclaimer,#add-page-options-disclaimer,#add-page-tracking-pin').off('click.activity').on('click.activity', function ()
			{
				$.ajax({
					type: "POST",
					url: window.BaseUrl + "statistic/writeActivity",
		...
		var runFavoritesPage = function (linkId, title, fileName, fileType)
		{
			...
			{
				$.SalesPortal.Favorites.addFavoriteLink(linkId);
			});
			...
			$.ajax({
				type: "POST",
				url: window.BaseUrl + "favorites/getFoldersList",
				beforeSend: function ()
				{
					$.mobile.loading('show', {
						textVisible: false,
