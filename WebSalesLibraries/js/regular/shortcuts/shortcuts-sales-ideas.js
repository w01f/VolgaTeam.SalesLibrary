(function ($) {
	...
	$.SalesPortal.ShortcutsSalesIdeas = function () {
		...
		this.init = function (data) {
			...
			servicePanel = $.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .service-panel');
			...
			$.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .service-panel').resizable({
				resizeHeight: false,
			...
			$(window).off('resize.sales-ideas').on('resize.sales-ideas', updateContentSize);
			updateContentSize();
		};
		...
		let saveItem = function (isSubmit, onSuccessHandler) {
			...
			{
				let itemContent = $.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .item-content');
				let allowSave = itemContent.find(">div.editable").length > 0 || shortcutData.options.isAdminRole;
				if (allowSave)
				{
					let selectedPlatforms = [];
					$.each(itemContent.find('.sales-ideas-item-details-platform:checked'), function () {
					...
					$.ajax({
						type: "POST",
						url: window.BaseUrl + "salesIdeas/saveItem",
						data: {
							...
							advertiser: $('#sales-ideas-item-advertiser').val(),
							revenue: $('#sales-ideas-item-revenue').val(),
							content: {
								nominationType: itemContent.find('.sales-ideas-item-nomination-type:checked').val(),
								seller: $('#sales-ideas-item-seller').val(),
								market: $('#sales-ideas-item-market').val(),
								station: $('#sales-ideas-item-station').val(),
								revenueType: itemContent.find('.sales-ideas-item-revenue-type:checked').val(),
								category: $('#sales-ideas-item-category').val(),
								mediaRevenue: $('#sales-ideas-item-revenue-media').val(),
								digitalRevenue: $('#sales-ideas-item-revenue-digital').val(),
								productionRevenue: $('#sales-ideas-item-revenue-production').val(),
								otherRevenue: $('#sales-ideas-item-revenue-other').val(),
								platforms: selectedPlatforms,
								teamMembers: $('#sales-ideas-item-team-members').val(),
								successStory: $('#sales-ideas-item-success-story').val(),
								milestones: $('#sales-ideas-item-milestones').val(),
							}
						},
						beforeSend: function () {
		...
		let initActionButtons = function () {
			...
			{
				shortcutActionsContainer.find('.sales-ideas-panel-show').hide();
			}
			else
				shortcutActionsContainer.find('.sales-ideas-panel-hide').hide();
			shortcutActionsContainer.find('.sales-ideas-panel-show').off('click.action').on('click.action', function () {
				servicePanel.show();
				shortcutActionsContainer.find('.sales-ideas-panel-hide').show();
				$(this).hide();
			...
			shortcutActionsContainer.find('.sales-ideas-panel-hide').off('click.action').on('click.action', function () {
				servicePanel.hide();
				shortcutActionsContainer.find('.sales-ideas-panel-show').show();
				$(this).hide();
			...
			shortcutActionsContainer.find('.sales-ideas-item-add').off('click.action').on('click.action', function () {
				itemList.addItem();
			});
			shortcutActionsContainer.find('.sales-ideas-item-delete').off('click.action').on('click.action', function () {
				itemList.deleteItem();
			});
			shortcutActionsContainer.find('.sales-ideas-item-save').off('click.action').on('click.action', function () {
				requestSaveItem();
			});
			shortcutActionsContainer.find('.sales-ideas-item-submit').off('click.action').on('click.action', function () {
				submitItem();
			});

			let itemListButtons = $.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .service-panel .item-list-buttons');
			itemListButtons.find('.item-list-add').off('click.sales-ideas').on('click.sales-ideas', function () {
				itemList.addItem();
			});
			itemListButtons.find('.item-list-save').off('click.sales-ideas').on('click.sales-ideas', function () {
				requestSaveItem()
			});
			itemListButtons.find('.item-list-submit').off('click.sales-ideas').on('click.sales-ideas', function () {
				submitItem();
			});

			let contentButtons = $.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .content-panel .content-buttons');
			contentButtons.find('.item-save').off('click.sales-ideas').on('click.sales-ideas', function () {
				requestSaveItem()
			});
			contentButtons.find('.item-submit').off('click.sales-ideas').on('click.sales-ideas', function () {
				submitItem();
			});

			$.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .service-panel .item-list-tabs .nav-tabs a[data-toggle="tab"]').on('shown.bs.tab', function () {
				itemList.load();
			});
		};
	...
	let ItemList = function () {
		...
		this.load = function (itemToSelectId) {
			...
			if ($('#sales-ideas-item-list-all').hasClass('active'))
				itemListConditions.listType = 'all-items';
			else if ($('#sales-ideas-item-list-archive').hasClass('active'))
				itemListConditions.listType = 'archive-items';
			else
				itemListConditions.listType = 'own-items';

			let itemList = $.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .service-panel .item-list-container');
			$.ajax({
				type: "POST",
				url: window.BaseUrl + "salesIdeas/getItemList",
		...
		this.addItem = function (parameters) {
			...
			$.ajax({
				type: "POST",
				url: window.BaseUrl + "salesIdeas/addItemDialog",
				data: {
				...
				success: function (msg) {
					...
					addItemContent.find('.btn.accept-button').on('click.sales-ideas', function () {
						$.ajax({
							type: "POST",
							url: window.BaseUrl + "salesIdeas/addItem",
							data: {
					...
					addItemContent.find('.btn.cancel-button').on('click.sales-ideas', function () {
						$.fancybox.close();
					});
					...
					buttons: [
						{
							...
							clickHandler: function () {
								...
								$.ajax({
									type: "POST",
									url: window.BaseUrl + "salesIdeas/deleteItem",
									data: {
		...
		this.updateContentSize = function () {
			let itemListContainer = $.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .service-panel .item-list-container');
			let itemListButtons = $.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .service-panel .item-list-buttons');
			let itemListTabs = $.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .service-panel .item-list-tabs');
		...
		let init = function (data) {
			...
			let tabHeaders = $.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .service-panel .item-list-tabs .nav-tabs');
			tabHeaders.find('a[href="#sales-ideas-item-list-own"]').html('my nominations (' + data.info.ownItemsCount + ')');
			tabHeaders.find('a[href="#sales-ideas-item-list-all"]').html('all nominations (' + data.info.allItemsCount + ')');
			tabHeaders.find('a[href="#sales-ideas-item-list-archive"]').html('archive (' + data.info.archiveItemsCount + ')');

			let tableIdentifier = 'sales-ideas-item-list';
			...
			table.on('contextmenu', '.item-data-row', function (event) {
				...
				{
					let menu = $('<ul class="dropdown-menu context-menu-content logger-form" role="menu" data-log-group="Sales Ideas" data-log-action="Sales Ideas Activity">' +
			...
			{
				$.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .content-buttons').hide();
				$.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .item-content').html('');
			}
		};
	...
	let ItemContent = function (parameters) {
		...
		this.updateContentSize = function () {
			let itemContent = $.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .item-content > div');
			let contentButtons = $.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .content-buttons');
		...
		let load = function () {
			...
			$.ajax({
				type: "POST",
				url: window.BaseUrl + "salesIdeas/getItemContent",
				data: {
					shortcutId: shortcutData.options.linkId,
		...
		let loadFiles = function (filesContainer, fileType, afterLoadCallback) {
			$.ajax({
				type: "POST",
				url: window.BaseUrl + "salesIdeas/getItemFiles",
				data: {
					itemId: that.itemId,
					fileType: fileType
				},
		...
		let afterLoad = function () {
			...
			let actionMenuSubmitButton = $('#shortcut-action-container').find('.sales-ideas-item-submit');
			let itemListSubmitButton = $.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .service-panel .item-list-buttons .item-list-submit');
			let bottomBarSubmitButton = $.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .content-panel .content-buttons .item-submit');
			if (that.isSubmitted)
			{
			...
			$('#sales-ideas-item-revenue').off('keypress.sales-ideas').on('keypress.sales-ideas',validateNumbers);
			$('#sales-ideas-item-revenue-digital').off('keypress.sales-ideas').on('keypress.sales-ideas', validateNumbers);
			$('#sales-ideas-item-revenue-media').off('keypress.sales-ideas').on('keypress.sales-ideas', validateNumbers);
			$('#sales-ideas-item-revenue-production').off('keypress.sales-ideas').on('keypress.sales-ideas', validateNumbers);
			$('#sales-ideas-item-revenue-other').off('keypress.sales-ideas').on('keypress.sales-ideas', validateNumbers);

			new PopupListEditor().init(itemContent.find('.sales-ideas-item-category-container'));
			new PopupListEditor().init(itemContent.find('.sales-ideas-item-market-container'));
			...
			{
				let attachmentsContainer = itemContent.find(".sales-ideas-item-attachments");
				let attachmentsDataContainer = $("#sales-ideas-item-attachments-data");
				let dropZoneObject = undefined;
				let abortUploading = false;
				attachmentsDataContainer.dropzone({
					url: window.BaseUrl + "salesIdeas/uploadFile?itemId=" + that.itemId + "&fileType=attachment",
					dictDefaultMessage: "",
			...
			$.SalesPortal.Content.getContentObject().find('.sales-ideas-main-page .content-buttons').show();
		...
		let afterAttachmentsLoad = function () {
			...
			attachmentsDataContainer.find('.file-item .file-name').off('click').on('click', function () {
				...
				let form = document.getElementById('form-download-sales-ideas-file');
				if (form === null)
				{
					form = document.createElement("form");
					form.setAttribute("id", "form-download-sales-ideas-file");
					form.setAttribute("method", "post");
					form.setAttribute("action", window.BaseUrl + 'salesIdeas/downloadFile');
					form._submit_function_ = form.submit;

					let hiddenField = document.createElement("input");
					hiddenField.setAttribute("id", "input-sales-ideas-file-id");
					hiddenField.setAttribute("type", "hidden");
					hiddenField.setAttribute("name", 'fileId');
					form.appendChild(hiddenField);
					document.body.appendChild(form);
				}
				document.getElementById('input-sales-ideas-file-id').setAttribute("value", fileId);
			...
			if (allowEdit)
			{
				attachmentsDataContainer.find('.file-item .file-delete').off('click').on('click', function () {
					...
					$.ajax({
						type: "POST",
						url: window.BaseUrl + "salesIdeas/deleteFile",
						data: {
							fileId: fileId,
						},
						beforeSend: function () {
