...
namespace FileManager.PresentationClasses.Cliparts
{
    ...
    {
        private static ClientLogosControl _instance = null;
        private bool _allowToPreviewImages = true;

        public static ClientLogosControl Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new ClientLogosControl();
                return _instance;
            }
        }

        private ClientLogosControl()
        {
            InitializeComponent();
        ...
        public void InitTreeList()
        {
            ...
            Thread thread = new Thread(new System.Threading.ThreadStart(delegate()
            {
                ...
                {
                    if (Directory.Exists(ConfigurationClasses.SettingsManager.Instance.ClientLogosRootPath))
                    {
                        DirectoryInfo rootFolder = new DirectoryInfo(ConfigurationClasses.SettingsManager.Instance.ClientLogosRootPath);
        ...
        private void UpdateButtonStatus()
        {
            ...
            if (treeListFiles.Selection.Count > 0)
            {
                ...
                if (selectedFolder != null)
                {
                    ...
                    if (selectedFolder.FullName.ToLower().Equals(ConfigurationClasses.SettingsManager.Instance.ClientLogosRootPath.ToLower()))
                    {
                        barButtonItemChangeNodeName.Enabled = false;
                        barButtonItemDeleteNode.Enabled = false;
                    }
