...
namespace FileManager.PresentationClasses.Cliparts
{
	...
	public partial class WebArtControl : UserControl
	{
		...
		public void InitTreeList()
		{
			...
			{
				if (!Directory.Exists(SettingsManager.Instance.WebArtRootPath)) return;
				var rootFolder = new DirectoryInfo(SettingsManager.Instance.WebArtRootPath);
		...
		private void UpdateButtonStatus()
		{
			...
			if (treeListFiles.Selection.Count > 0)
			{
				...
				if (selectedFolder != null)
				{
					...
					if (selectedFolder.FullName.ToLower().Equals(SettingsManager.Instance.WebArtRootPath.ToLower()) ||
						selectedFolder.FullName.ToLower().Equals(SettingsManager.Instance.AdSpecsSamplesRootPath.ToLower()) ||
						selectedFolder.FullName.ToLower().Equals(SettingsManager.Instance.ScreenshotLibraryRootPath.ToLower()))
					{
						barButtonItemChangeNodeName.Enabled = false;
						barButtonItemDeleteNode.Enabled = false;
					}
		...
		private void treeListFiles_DragOver(object sender, DragEventArgs e)
		{
			...
			if (node != null && e.Data.GetDataPresent(DataFormats.Serializable, true))
			{
				...
				if (node.Tag.GetType() == typeof(DirectoryInfo))
				{
					...
					if (parentFolder != null && !parentFolder.FullName.Equals(SettingsManager.Instance.WebArtRootPath))
					{
						node.Expanded = true;
						treeListFiles.FocusedNode = node;
						node.Selected = true;
		...
		private void treeListFiles_DragDrop(object sender, DragEventArgs e)
		{
			...
			if (node != null && e.Data.GetDataPresent(DataFormats.Serializable, true))
			{
				...
				if (data != null && parentFolder != null && !parentFolder.FullName.Equals(SettingsManager.Instance.WebArtRootPath))
				{
