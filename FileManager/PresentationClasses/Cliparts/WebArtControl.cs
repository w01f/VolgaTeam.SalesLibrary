...
namespace FileManager.PresentationClasses.Cliparts
{
    ...
    public partial class WebArtControl : UserControl
    {
        private static WebArtControl _instance = null;
        private bool _allowToPreviewImages = true;

        public static WebArtControl Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new WebArtControl();
                return _instance;
            }
        }

        private WebArtControl()
        {
            InitializeComponent();
        ...
        public void InitTreeList()
        {
            ...
            Thread thread = new Thread(new System.Threading.ThreadStart(delegate()
            {
                ...
                {
                    if (Directory.Exists(ConfigurationClasses.SettingsManager.Instance.WebArtRootPath))
                    {
                        DirectoryInfo rootFolder = new DirectoryInfo(ConfigurationClasses.SettingsManager.Instance.WebArtRootPath);
        ...
        private void UpdateButtonStatus()
        {
            ...
            if (treeListFiles.Selection.Count > 0)
            {
                ...
                if (selectedFolder != null)
                {
                    ...
                    if (selectedFolder.FullName.ToLower().Equals(ConfigurationClasses.SettingsManager.Instance.WebArtRootPath.ToLower()) ||
                        selectedFolder.FullName.ToLower().Equals(ConfigurationClasses.SettingsManager.Instance.AdSpecsSamplesRootPath.ToLower()) ||
                        selectedFolder.FullName.ToLower().Equals(ConfigurationClasses.SettingsManager.Instance.ScreenshotLibraryRootPath.ToLower()))
                    {
                        barButtonItemChangeNodeName.Enabled = false;
                        barButtonItemDeleteNode.Enabled = false;
                    }
        ...
        private void treeListFiles_DragOver(object sender, DragEventArgs e)
        {
            ...
            if (node != null && e.Data.GetDataPresent(DataFormats.Serializable, true))
            {
                ...
                if (node.Tag.GetType() == typeof(DirectoryInfo))
                {
                    ...
                    if (parentFolder != null && !parentFolder.FullName.Equals(ConfigurationClasses.SettingsManager.Instance.WebArtRootPath))
                    {
                        node.Expanded = true;
                        treeListFiles.FocusedNode = node;
                        node.Selected = true;
        ...
        private void treeListFiles_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if (node != null && e.Data.GetDataPresent(DataFormats.Serializable, true))
            {
                ...
                if (data != null && parentFolder != null && !parentFolder.FullName.Equals(ConfigurationClasses.SettingsManager.Instance.WebArtRootPath))
                {
