...
namespace FileManager.BusinessClasses
{
	public class SettingsManager
	{
		...
		private string _categoryRequestSettingsPath = string.Empty;
		private string _ribbonSettingsFilePath = string.Empty;
		private string _dashboardSyncSettingsFilePath = string.Empty;
		private string _salesDepotSyncSettingsFilePath = string.Empty;
		...
		private void Init()
		{
			...
			var appDataNameSettingFile = Path.Combine(ApplicationRootPath, "app_data.txt");
			var appDataName = File.Exists(appDataNameSettingFile) ? File.ReadAllText(appDataNameSettingFile) : "FileManager";

			ApplicationLocalDataPath = string.Format(@"{0}\{1}", Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), appDataName);
			...
			{
				SettingsRootPath = string.Format(@"{0}\newlocaldirect.com\xml", Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles));
				if (!Directory.Exists(SettingsRootPath))
					Directory.CreateDirectory(SettingsRootPath);
			}

			var settingsFolderPath = AppModeManager.Instance.AppMode == AppModeEnum.Local ? string.Format(@"{0}\file_manager", SettingsRootPath) : ApplicationLocalDataPath;
			...
			else if (AppModeManager.Instance.AppMode == AppModeEnum.Cloud)
				TempPath = String.Format(@"{0}\Temp", ApplicationLocalDataPath);
