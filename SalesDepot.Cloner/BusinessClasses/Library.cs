...
namespace SalesDepot.CoreObjects.BusinessClasses
{
	public class Library : ILibrary
	{
		...
		{
			foreach (LibraryLink file in Pages.SelectMany(page => page.Folders.SelectMany(folder => folder.Files.Cast<LibraryLink>())))
			{
		...
		{
			var library = new Library(name, folder);

			library.UseDirectAccess = UseDirectAccess;
			library.DirectAccessFileBottomDate = DirectAccessFileBottomDate;
			library.BrandingText = BrandingText;

			library.ApplyAppearanceForAllWindows = ApplyAppearanceForAllWindows;
			library.ApplyWidgetForAllWindows = ApplyWidgetForAllWindows;
			library.ApplyBannerForAllWindows = ApplyBannerForAllWindows;
			library.MinimizeOnSync = MinimizeOnSync;
			library.CloseAfterSync = CloseAfterSync;
			library.ShowProgressDuringSync = ShowProgressDuringSync;
			library.EnableInactiveLinks = EnableInactiveLinks;
			library.InactiveLinksBoldWarning = InactiveLinksBoldWarning;
			library.ReplaceInactiveLinksWithLineBreak = ReplaceInactiveLinksWithLineBreak;
			library.InactiveLinksMessageAtStartup = InactiveLinksMessageAtStartup;
			library.SendEmail = SendEmail;
			library.ExtraFolders.AddRange(ExtraFolders.Select(x => x.Clone(library)));
			library.Pages.AddRange(Pages.Select(x => x.Clone(library)));
			library.EmailList.AddRange(EmailList);
			library.AutoWidgets.AddRange(AutoWidgets);
			library.PreviewContainers.AddRange(PreviewContainers.Select(x => x.Clone(library)));

			library.EnableAutoSync = EnableAutoSync;
			library.SyncScheduleRecords.AddRange(SyncScheduleRecords);

			library.EnableProgramManagerSync = EnableProgramManagerSync;
			library.ProgramManagerLocation = ProgramManagerLocation;

			library.OvernightsCalendar = OvernightsCalendar.Clone(library);
			library.IPadManager = IPadManager.Clone(library);

			library.IsConfigured = IsConfigured;
