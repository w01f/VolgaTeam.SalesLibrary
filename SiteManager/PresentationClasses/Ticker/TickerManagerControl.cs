	...
	public partial class TickerManagerControl : UserControl
	{
		...
		private void SetData(ref string updateMessage)
		{
			...
			using (var form = new FormProgress())
			{
				...
				form.TopMost = true;
				var thread = new Thread(() => BusinessClasses.SiteManager.Instance.SelectedSite.SetTickerLinks(_records.ToArray(), out message));
				form.Show();
				thread.Start();
		...
		public void RefreshData(bool showMessages)
		{
			...
			if (showMessages)
			{
				using (var form = new FormProgress())
				{
					...
					form.TopMost = true;
					var thread = new Thread(() => _records.AddRange(BusinessClasses.SiteManager.Instance.SelectedSite.GetTickerLinks(out message)));
					form.Show();
					thread.Start();
					while (thread.IsAlive)
					{
