...
{
	[ToolboxItem(false)]
	public partial class QuizPassGroupReportControl : UserControl, IActivitiesView
	{
		private readonly List<QuizPassGroupReportRecord> _records = new List<QuizPassGroupReportRecord>();
		...
		public QuizPassGroupReportControl()
		{
			InitializeComponent();
			Dock = DockStyle.Fill;
		...
		public void UpdateData(bool showMessages, ref string updateMessage)
		{
			...
			if (showMessages)
			{
				using (var form = new FormProgress())
				{
					...
					form.TopMost = true;
					var thread = new Thread(() => _records.AddRange(BusinessClasses.WebSiteManager.Instance.SelectedSite.GetQuizPassGroupReport(StartDate, EndDate, out message)));
					form.Show();
					thread.Start();
		...
		public void ExportData()
		{
			using (var dialog = new SaveFileDialog())
			{
				...
				dialog.FileName = string.Format("StationQuizPerformanceReport({0}).xlsx", DateTime.Now.ToString("MMddyy-hmmtt"));
				dialog.Filter = "Excel files|*.xlsx";
				dialog.Title = "Station Quiz Performance Report";
		...
		{
			var reportHeader = string.Format("Station Quiz Performance Report: {0} - {1}", StartDate.ToString("MM/dd/yy"), EndDate.AddDays(-1).ToString("MM/dd/yy"));
