	...
	public partial class NavigationGroupReportControl : UserControl, IActivitiesView
	{
		...
		public void UpdateData(bool showMessages, ref string updateMessage)
		{
			...
			if (showMessages)
			{
				using (var form = new FormProgress())
				{
					...
					form.TopMost = true;
					var thread = new Thread(() => _records.AddRange(BusinessClasses.WebSiteManager.Instance.SelectedSite.GetNavigationGroupReport(StartDate, EndDate, out message)));
					form.Show();
					thread.Start();
		...
		public void ExportData()
		{
			using (var dialog = new SaveFileDialog())
			{
				...
				dialog.FileName = string.Format("GroupActivity({0}).xlsx", DateTime.Now.ToString("MMddyy-hmmtt"));
