...
{
	[ToolboxItem(false)]
	public partial class AccessGroupReportControl : UserControl, IActivitiesView
	{
		...
		private AccessGroupFilter _filterControl;
		public Control FilterControl
		{
			get { return _filterControl; }
		}

		public AccessGroupReportControl()
		{
			InitializeComponent();
			Dock = DockStyle.Fill;
			_filterControl = new AccessGroupFilter();
		...
		public void ExportData()
		{
			using (var dialog = new SaveFileDialog())
			{
				...
				dialog.FileName = string.Format("GroupAccess({0}).xls", DateTime.Now.ToString("MMddyy-hmmtt"));
		...
		{
			var reportHeader = string.Format("Individual User Group Site Access Report: {0} - {1}", StartDate.ToString("MM/dd/yy"), EndDate.AddDays(-1).ToString("MM/dd/yy"));
